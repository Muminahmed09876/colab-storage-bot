...
async def compress_video(input_path: Path, output_path: Path, target_bitrate_kbps: int, duration_sec: int) -> bool:
    ...
    def sync_compress():
        # Changed preset to medium for normal convert
        cmd = [
...
async def run_youtube_downloader(is_tg_upload: bool, client: Client, user_id: int, target_chat: str, progress_callback):
    print("\n❌ YouTube automation feature has been disabled/removed per your recent ledger instructions.")
    print("Please use the 'd' or 'convert'/'co' command with direct links instead.")
    return

async def command_mode(client: Client):
    global GLOBAL_CONFIG
    print("\n--- 💻 Command Mode ---")
    print("Cmds: set_thum <path/time> | del_thum | set_cap | upload | upload_mkv | convert (co) | convert tg (cotg) | youtube or yt | youtubetg or ytg | d")
    ...
    while True:
        try:
            ...
            elif cmd in ['convert', 'co', 'cotg']:
                is_up = False
                if cmd == 'cotg' or (len(args) > 0 and args[0].lower() == 'tg'):
                    is_up = True
                ...
                while True:
                    ...
                    if target_arg.lower() == 'ok':
                        ...
                        for item in convert_queue:
                            ...
                            for job in item['jobs']:
                                final_jobs_to_process.append({
                                    ...
                                    'res': job['res'],
                                    'audio_indices': item.get('audio_indices', []),
                                    'audio_kbps': item.get('audio_kbps', 128),
                        ...
                        async def convert_job_task(job_idx, job_data):
                            ...
                            res = job_data['res']
                            a_indices = job_data['audio_indices']
                            a_kbps = job_data['audio_kbps']
                            
                            metadata = get_video_metadata(f)
                            dur = metadata.get('duration', 0)
                            
                            total_a_streams = len(get_audio_stream_info(f))
                            actual_a_count = len(a_indices) if a_indices else total_a_streams
                            target_ext = '.mkv' if actual_a_count > 1 or f.suffix.lower() == '.mkv' else '.mp4'
                            ...
                            if t_b == -1 and res == 0:
                                ...
                                try:
                                    if a_indices and len(a_indices) < total_a_streams:
                                        await modify_audio_tracks_and_copy(f, f_up, [x+1 for x in a_indices], dur)
                                    else:
                                ...
                                v_bitrate_kbps = max(50, math.ceil(((t_b * 8) / dur) / 1000) - a_kbps)
                                c_out = TMP / f"comp_{uid}{target_ext}"
                                print(f"\n[Job {job_idx+1}] Compressing {f.name} -> {res}p, ~{t_b/(1024*1024):.1f}MB (Video: {v_bitrate_kbps}kbps, Audio: {a_kbps}kbps)...")
                                
                                def sync_compress_adv():
                                    ...
                                    if not a_indices:
                                        ...
                                        for i_a in a_indices:
                                            cmd.extend(["-map", f"0:a:{i_a}"])
                                            
                                    cmd.extend(["-c:v", "libx264"])
                                    if v_bitrate_kbps > 0:
                                        cmd.extend(["-b:v", f"{v_bitrate_kbps}k"])
                                    if res > 0:
                                    ...
                                    cmd.extend(["-c:a", "aac"])
                                    if a_kbps > 0:
                                        cmd.extend(["-b:a", f"{a_kbps}k"])
                                        
                                    cmd.append(str(c_out))
                                    try:
                                        run_ffmpeg_command_with_progress(cmd, dur, f"Convert {res}p")
                                        return c_out.exists() and c_out.stat().st_size > 0
                                    except: return False
                                
                                success = await asyncio.to_thread(sync_compress_adv)
                                if success:
                    ...
                    if target_arg.startswith(('http://', 'https://')):
                        print(f"🔗 Downloading URL First: {target_arg} ...")
                        dl_dir = TMP / uuid.uuid4().hex
                        dl_dir.mkdir(parents=True, exist_ok=True)
                        try:
                            subprocess.run(["wget", "-q", "--show-progress", "--content-disposition", "-P", str(dl_dir), target_arg], check=True)
                            downloaded_files = list(dl_dir.iterdir())
                            if downloaded_files:
                                target_list.append(str(downloaded_files[0]))
                                is_url_download = True
                            else:
                                print("❌ No file found after download.")
                                continue
                        except Exception as e:
                            print(f"❌ Download failed for {target_arg}: {e}")
                            continue
                    else:
                    ...
                    for t in target_list:
                        ...
                        dur = metadata.get('duration', 0)
                        a_s = get_audio_stream_info(f_path)
                        ...
                        for ix, tr in enumerate(a_s):
                            approx_a_bytes = int((128 * 1000 / 8) * dur) if dur > 0 else 0
                            audio_total_bytes += approx_a_bytes
                            print(f" [{ix+1}] {tr['description']} | Approx Size: {approx_a_bytes / (1024*1024):.2f} MB")
                            
                        est_vid_size = max(0, file_size - audio_total_bytes)
                        print(f"🎥 Video Stream Size (Approx): {est_vid_size / (1024*1024):.2f} MB")
                        
                        sel_a = input("\nSelect Audio Tracks (e.g. 1,2 or Enter for all): ").strip()
                        selected_audios = []
                        if sel_a:
                            selected_audios = [int(x)-1 for x in sel_a.split(',') if x.strip().isdigit() and int(x) <= len(a_s)]
                            
                        a_kbps = 128
                        a_q = input("\nSelect Audio Quality:\n1) 128k (Normal)\n2) 96k\n3) 64k\n4) 48k\nChoice [Default 1]: ").strip()
                        if a_q == '2': a_kbps = 96
                        elif a_q == '3': a_kbps = 64
                        elif a_q == '4': a_kbps = 48
                        
                        print("\nVideo Quality Options: 144p, 240p, 360p, 480p, 720p, 1080p")
                        print("Format: <res> <size>, e.g. 480p 50mb, 720p 150mb")
                        print("Enter 'O' for original size and resolution.")
                        v_jobs_input = input("Enter conversions: ").strip()
                        
                        jobs = []
                        if v_jobs_input.upper() == 'O':
                            jobs.append({'res': 0, 'target_bytes': -1})
                        else:
                            for job_str in v_jobs_input.split(','):
                                job_str = job_str.strip()
                                if not job_str: continue
                                parts = job_str.split()
                                if len(parts) >= 2:
                                    res_str = parts[0].lower().replace('p', '')
                                    sz_str = parts[1]
                                    try:
                                        res = int(res_str)
                                        t_b = parse_size(sz_str)
                                        if t_b > 0: jobs.append({'res': res, 'target_bytes': t_b})
                                    except: pass
                                    
                        if not jobs:
                            print("⚠️ No valid conversion formats provided. Skipping file.")
                            continue
                            
                        convert_queue.append({
                            'target': str(f_path),
                            'audio_indices': selected_audios,
                            'audio_kbps': a_kbps,
                            'jobs': jobs,
                            'is_url': is_url_download
                        })
